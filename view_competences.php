...
echo $OUTPUT->box(text_to_html(get_string("explaincompetencesoverview", "block_exacomp")));
...
foreach ($course_desriptors as $coures_descriptor) {
	...
	$content.='<tr><td>'.$coures_descriptor->fullname.'</td><td>'.count($coures_descriptor->descriptors).'</td><td>'.count(block_exacomp_get_usercompetences($USER->id, 1, $coures_descriptor->id)).'</td><td>'.block_exacomp_get_ladebalken($coures_descriptor->id, $USER->id, count($coures_descriptor->descriptors)).'</td></tr>';
...
$content.='<tr><td><b>Total</b></td><td>'.$total.'</td><td>'.$total_achieved.'</td><td>'.block_exacomp_get_ladebalken($coures_descriptor->id, $USER->id, $total,$total_achieved).'</td></tr></table>';
foreach ($course_desriptors as $coures_descriptor) {
    $descriptors = $coures_descriptor->descriptors;
    if ($descriptors) {
		...
		//$content .= block_exacomp_get_ladebalken($coures_descriptor->id, $USER->id, count($descriptors));
    }
}
...
$descriptors = block_exacomp_check_portfolio_competences($USER->id);
	if ($descriptors) {
	    ...
	    foreach ($descriptors as $descriptor) {
	        ...
	        $icon = block_exacomp_check_teacher_assign($descriptor,$USER->id);
	        $content .= '<td>'.$icon.'</td>';
	        $icon = block_exacomp_check_student_assign($descriptor,$USER->id);
