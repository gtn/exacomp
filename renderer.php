...
class block_exacomp_renderer extends plugin_renderer_base {
    ...
    public function subject_dropdown($schooltypetree, $selectedSubject) {
        ...
        foreach ($schooltypetree as $schooltype) {
            ...
            foreach ($schooltype->subjects as $subject) {
                $options[$schooltype->title][$subject->id] = s($subject->title);
            }
        ...
        foreach ($subjects as $subject) {
            ...
            $content .= html_writer::tag('li',
                html_writer::link(
                    ...
                    s($subject->title) . '<span class="no-br">' . $extra . '</span>', [
            ...
            foreach ($subject->topics as $topic) {
                if (block_exacomp_is_teacher() || block_exacomp_is_topic_visible($COURSE->id, $topic, g::$USER->id)) {
                    ...
                    ]), block_exacomp_get_topic_numbering($topic) . ' ' . s($topic->title) . '<span class="no-br">' . $extra . '</span>', array(
    ...
    public function overview_metadata($schooltype, $subject, $descriptor, $cat) {
        ...
        $cell->text = html_writer::span(block_exacomp_get_string('subject_singular'), 'exabis_comp_top_name')
            . html_writer::div(s($schooltype), 'exabis_comp_top_value');

        $row->cells[] = $cell;

        $cell = new html_table_cell();
        $cell->text = html_writer::span(block_exacomp_get_string('comp_field_idea'), 'exabis_comp_top_name')
            . html_writer::div($subject ? (!empty($subject->numb) ? s($subject->numb) . " - " : '') . s($subject->title) : '', 'exabis_comp_top_value');
        ...
        if ($descriptor) {
            ...
            $cell->text = html_writer::span(block_exacomp_get_string('comp'), 'exabis_comp_top_name')
                . html_writer::div(s($descriptor->title), 'exabis_comp_top_value');
        ...
        if (block_exacomp_is_numbering_enabled()) {
            ...
            $cell->text = html_writer::span(block_exacomp_get_string('progress'), 'exabis_comp_top_name')
                . html_writer::div($cat ? s($cat->title) : '', 'exabis_comp_top_value');
    ...
    public function competence_grid($niveaus, $skills, $topics, $data, $selection = array(), $courseid = 0, $studentid = 0, $subjectid = 0, $target = '') {
        ...
        $scheme = ($courseid == 0) ? 1 : block_exacomp_get_assessment_comp_scheme($courseid);
        ...
        $scheme_values = global_config::get_teacher_eval_items($courseid, false, $scheme);
        ...
        foreach ($data as $skillid => $skill) {
            ...
            foreach ($skill as $topicid => $topic) {
                ...
                $cellTopic->text = html_writer::tag("span", html_writer::tag("span", block_exacomp_get_topic_numbering(topic::get($topicid)) . " " . $topics[$topicid], array('class' => 'rotated-text__inner')),
                    array('class' => 'rotated-text'));
                ...
                foreach ($niveaus as $niveauid => $niveau) {
                    if (isset($data[$skillid][$topicid][$niveauid])) {
                        ...
                        foreach ($data[$skillid][$topicid][$niveauid] as $descriptor) {
                            ...
                            $text = block_exacomp_get_descriptor_numbering($descriptor) . " " . s($descriptor->title);
                            ...
                            if ($studentid > 0) {
                                ...
                                $item = descriptor::get($descriptor->id);
                                $eval = block_exacomp_get_comp_eval_merged($courseid, $studentid, $item);
                                if (block_exacomp_get_assessment_SelfEval(BLOCK_EXACOMP_TYPE_DESCRIPTOR, $courseid)) {
                                ...
                                switch ($scheme) {
                                    ...
                                    case BLOCK_EXACOMP_ASSESSMENT_TYPE_VERBOSE:
                                        ...
                                        $teacher_eval_items = global_config::get_teacher_eval_items(g::$COURSE->id, true, BLOCK_EXACOMP_ASSESSMENT_TYPE_VERBOSE);
        ...
        foreach ($subjects as $subject) {
            ...
            foreach ($students as $student) {
                ...
                if (!$this->is_print_mode() && block_exacomp_is_exastud_installed() && ($info = api::get_student_review_link_info_for_teacher($student->id))) {
                    $studentCell->text .= ' <a href="' . $info->url . '" title="' . block_exacomp_trans('de:Überfachliche Bewertung') .
                        '" onclick="window.open(this.href,this.target,\'width=880,height=660,scrollbars=yes\'); return false;">' . '<img src="pix/review_student.png" />' . '</a>';
                }
            ...
            if ($first) {
                if ($crosssubjid) {
                    ...
                    $crosssubjectObj = cross_subject::get($crosssubjid, MUST_EXIST);
                    ...
                    $crosstitleCell->text = s($crosssubjectObj->title);
                    $crossubjtitleRow->cells[] = $crosstitleCell;
                    $crossubjtitleRow->cells[] = new html_table_cell();
                    foreach ($students as $student) {
                        if ($role == BLOCK_EXACOMP_ROLE_TEACHER) {
                            $reviewerid = $DB->get_field(BLOCK_EXACOMP_DB_COMPETENCES, "reviewerid",
                                array("userid" => $student->id, "compid" => $crosssubjid, "courseid" => $courseid, "role" => BLOCK_EXACOMP_ROLE_TEACHER, "comptype" => BLOCK_EXACOMP_TYPE_CROSSSUB));
                            if ($reviewerid == $USER->id || $reviewerid == 0) {
                                $reviewerid = null;
                            }
                        }
                        ...
                        if ($this->useEvalNiveau && block_exacomp_get_assessment_theme_diffLevel($courseid) == 1) {
                        ...
                        if (block_exacomp_get_assessment_theme_SelfEval($courseid) == 1) {
                            // Only emojis?
                        ...
                        if ($role == BLOCK_EXACOMP_ROLE_STUDENT) {
                            if ($this->useEvalNiveau && $showevaluation && $this->diffLevelExists) {
                                $crossubjtitleRow->cells[] = $niveau_cell;
                            }
                            if ($showevaluation) {
                                $crossubjtitleRow->cells[] = $teacher_evaluation_cell;
                            }
                            $crossubjtitleRow->cells[] = $student_evaluation_cell;
                        } else { // Teacher
                            if ($showevaluation) {
                                $crossubjtitleRow->cells[] = $student_evaluation_cell;
                            }
                    ...
                    $example_scheme = block_exacomp_get_assessment_example_scheme($courseid);
                    ...
                    foreach ($examples_crosssubj as $example) {
                        ...
                        $titleCell->text = html_writer::div(html_writer::tag('span', s($example->title)), '', ['title' => $title]);
                        ...
                        if (!$this->is_print_mode()) {
                            ...
                            if ($editmode) {
                                ...
                                $titleCell->text .= html_writer::link("#", $this->pix_icon("t/up", block_exacomp_get_string('up')),
                                    array("exa-type" => "example-sorting", 'exa-direction' => 'up', "exa-exampleid" => $example->id, "exa-descrid" => @$descriptor->id));
                                $titleCell->text .= html_writer::link("#", $this->pix_icon("t/down", block_exacomp_get_string('down')),
                                    array("exa-type" => "example-sorting", 'exa-direction' => 'down', "exa-exampleid" => $example->id, "exa-descrid" => @$descriptor->id));
                            ...
                            if ($isEditingTeacher && ($role == BLOCK_EXACOMP_ROLE_TEACHER) && ($editmode || (!$editmode && $one_student && block_exacomp_is_example_visible($courseid, $example, 0)))) {
                                if ($example_used) {
                                    $titleCell->text .= html_writer::span($this->local_pix_icon("visibility_lock.png", block_exacomp_get_string('competence_locked'), array('height' => '18')), 'imglocked',
                        ...
                        foreach ($students as $student) {
                            ...
                            if ($this->useEvalNiveau && block_exacomp_get_assessment_example_diffLevel($courseid) == 1) {
                                $niveau_cell->text = $this->generate_niveau_select(
                                    'niveau_examples',
                                    $example->id,
                                    'examples',
                            ...
                            if (block_exacomp_get_assessment_example_SelfEval($courseid) == 1) {
                                // Only emojis?
                                $student_evaluation_cell->text = $this->generate_select(
                                    $checkboxname,
                                    $example->id,
            ...
            if (!$crosssubjid) {
                ...
                $title->text = html_writer::tag("b", s($subject->title));
                ...
                foreach ($students as $student) {
                    if ($role == BLOCK_EXACOMP_ROLE_TEACHER) {
                        // TEACHER VIEW.
                        $reviewerid = $DB->get_field(BLOCK_EXACOMP_DB_COMPETENCES, "reviewerid",
                            array("userid" => $student->id, "compid" => $subject->id, "courseid" => $courseid, "role" => BLOCK_EXACOMP_ROLE_TEACHER, "comptype" => BLOCK_EXACOMP_TYPE_SUBJECT));
                        if ($reviewerid == $USER->id || $reviewerid == 0) {
                            $reviewerid = null;
                        }
                        ...
                        if ($this->useEvalNiveau && block_exacomp_get_assessment_subject_diffLevel($courseid)) {
                        ...
                        $subjectscheme = block_exacomp_get_assessment_subject_scheme($courseid);
                        $evaluation_cell->text = '';

                        if (block_exacomp_get_assessment_subject_SelfEval($courseid) == 1) {
                        ...
                        if (@$subject->isglobal && block_exacomp_get_settings_by_course($courseid)->isglobal && block_exacomp_is_dakora_teacher($USER->id)) {
                            if (array_key_exists($subject->id, @$student->subjects->globalgradings) && @$student->subjects->globalgradings[$subject->id] != "") {
                                ...
                                $globalgradings = html_writer::tag('b', ' ' . $this->pix_icon("i/groupevent", @$student->subjects->globalgradings[$subject->id]) . ' ',
                                    array('id' => 'globalgradings', 'descrid' => $subject->id, 'studentid' => $student->id));
                        ...
                        if (isset($student->subjects->gradinghistory[$subject->id]) && substr_count($student->subjects->gradinghistory[$subject->id], '<br>') > 1) {
                            ...
                            $gradinghistory =
                                html_writer::tag('b', ' ' . $this->pix_icon("i/duration", $student->subjects->gradinghistory[$subject->id]) . ' ', array('id' => 'gradinghistory', 'descrid' => $subject->id, 'studentid' => $student->id));
    ...
    public function topics(&$rows, $level, $topics, $data, $students, $profoundness = false, $editmode = false, $crosssubjid = 0, $isEditingTeacher = true, $forReport = false, $hideAllActionButtons = false) {
        ...
        if (block_exacomp_is_topicgrading_enabled($data->courseid) || count($topics) > 0 || $topicparam == BLOCK_EXACOMP_SHOW_ALL_TOPICS) {
            // display topic row
        ...
        foreach ($topics as $topic) {
            ...
            if ($data->role == BLOCK_EXACOMP_ROLE_TEACHER || $visible) {
                ...
                if ($isEditingTeacher && !$forReport && ($data->role == BLOCK_EXACOMP_ROLE_TEACHER) && ($editmode || (!$editmode && $one_student && block_exacomp_is_topic_visible($data->courseid, $topic, 0)))) {
                    if ($topic_used) {
                        $outputname .= html_writer::span($this->local_pix_icon("visibility_lock.png", block_exacomp_get_string('competence_locked'), array('height' => '18')), 'imglocked',
                            array('title' => block_exacomp_get_string('competence_locked')));
                    } else {
                ...
                if ($profoundness) {
                    ...
                    foreach ($students as $student) {
                        if ($data->role == BLOCK_EXACOMP_ROLE_TEACHER) {
                            $reviewerid = $DB->get_field(BLOCK_EXACOMP_DB_COMPETENCES, "reviewerid",
                                array("userid" => $student->id, "compid" => $topic->id, "courseid" => $data->courseid, "role" => BLOCK_EXACOMP_ROLE_TEACHER, "comptype" => BLOCK_EXACOMP_TYPE_TOPIC));
                            if ($reviewerid == $USER->id || $reviewerid == 0) {
                                $reviewerid = null;
                            }
                        }
                        ...
                        if ($this->useEvalNiveau && block_exacomp_get_assessment_topic_diffLevel($data->courseid) == 1) {
                            $niveau_cell->text = $this->generate_niveau_select(
                                'niveau_topic',
                                $topic->id,
                                'topics',
                        ...
                        // student show evaluation
                        if (block_exacomp_additional_grading(BLOCK_EXACOMP_TYPE_TOPIC, $data->courseid)) {
                            switch ($data->scheme) {
                                case BLOCK_EXACOMP_ASSESSMENT_TYPE_NONE: // None.
                                    $teacher_evaluation_cell->text = '';
                                    break;
                            ...
                            if (@$data->subject->isglobal && block_exacomp_get_settings_by_course($COURSE->id)->isglobal && block_exacomp_is_dakora_teacher($USER->id)) {
                                if (array_key_exists($topic->id, @$student->topics->globalgradings) && @$student->topics->globalgradings[$topic->id] != "") {
                                    //Add the other globalgradings as tooltipp
                                    $globalgradings = html_writer::tag('b', ' ' . $this->pix_icon("i/groupevent", @$student->topics->globalgradings[$topic->id]) . ' ',
                                        array('id' => 'globalgradings', 'descrid' => $topic->id, 'studentid' => $student->id));
                            ...
                            if (isset($student->topics->gradinghistory[$topic->id]) && substr_count($student->topics->gradinghistory[$topic->id], '<br>') > 1) {
                                $gradinghistory =
                                    html_writer::tag('b', ' ' . $this->pix_icon("i/duration", $student->topics->gradinghistory[$topic->id]) . ' ', array('id' => 'gradinghistory', 'descrid' => $topic->id, 'studentid' => $student->id));
                                $teacher_evaluation_cell->text .= $gradinghistory;
                            }
                        }

                        if (block_exacomp_get_assessment_topic_SelfEval($data->courseid) == 1) {
                            // Only emojis?
                            $student_evaluation_cell->text = $this->generate_select(
                                $checkboxname,
                                $topic->id,
    ...
    function descriptors(&$rows, $level, $descriptors, $data, $students, $profoundness = false, $editmode = false, $custom_created_descriptors = false, $parent = false, $crosssubjid = 0, $parent_visible = array(), $isEditingTeacher = true,
        ...
        foreach ($descriptors as $descriptor) {
            ...
            if ($data->role == BLOCK_EXACOMP_ROLE_TEACHER || $visible) {
                ...
                $titleCell->text =
                    html_writer::div(html_writer::tag('span', $outputname . ($title ? $this->pix_icon("i/info", $title) : '') . $this->submission_icon($data->courseid, $descriptor->id, $USER->id, false, BLOCK_EXACOMP_TYPE_DESCRIPTOR)), '');
                //                $titleCell->text .= $this->submission_icon($data->courseid, $descriptor->id, $USER->id, false, BLOCK_EXACOMP_TYPE_DESCRIPTOR);
                ...
                if (!$this->is_print_mode() && !$forReport) {
                    ...
                    if ($editmode && $isEditingTeacher && $data->role == BLOCK_EXACOMP_ROLE_TEACHER
                        ...
                        $titleCell->text .= html_writer::link('#', $this->pix_icon('t/up', block_exacomp_get_string('move_up')),
                            array('exa-type' => 'descriptor-sorting', 'exa-direction' => 'up', 'exa-descriptorid' => $descriptor->id, 'exa-topicid' => $descriptor->topicid));
                        $titleCell->text .= html_writer::link('#', $this->pix_icon('t/down', block_exacomp_get_string('move_down')),
                            array('exa-type' => 'descriptor-sorting', 'exa-direction' => 'down', 'exa-descriptorid' => $descriptor->id, 'exa-topicid' => $descriptor->topicid));
                    }
                }
                /*if ($editmode) {
                ...
                foreach ($students as $student) {
                    ...
                    if ($data->role == BLOCK_EXACOMP_ROLE_TEACHER) {
                        $reviewerid = $DB->get_field(BLOCK_EXACOMP_DB_COMPETENCES, "reviewerid",
                            array("userid" => $student->id, "compid" => $descriptor->id, "courseid" => $data->courseid, "role" => BLOCK_EXACOMP_ROLE_TEACHER, "comptype" => BLOCK_EXACOMP_TYPE_DESCRIPTOR));
                        if ($reviewerid == $USER->id || $reviewerid == 0) {
                            $reviewerid = null;
                        }
                    }
                    ...
                    } else if ($visible && !$one_student && !$editmode) {
                    ...
                    if ($data->exaport_exists) {
                        if (isset($data->eportfolioitems[$student->id]) && isset($data->eportfolioitems[$student->id]->competencies[$descriptor->id])) {
                            ...
                            $eportfoliotext = '<span title="' . s($text) . '" class="exabis-tooltip">' . $img . '</span>';
                        } else {
                            $eportfoliotext = '';
                        }
                    }
                    ...
                    if (!$profoundness) {
                        ...
                        if (($this->useEvalNiveau && block_exacomp_get_assessment_comp_diffLevel($data->courseid) == 1 && $level == 1)
                            || ($this->useEvalNiveau && block_exacomp_get_assessment_childcomp_diffLevel($data->courseid) == 1) && $level == 2) {
                            $niveau_cell->text = $this->generate_niveau_select(
                                'niveau_descriptor',
                                $descriptor->id,
                                'competencies',
                        ...
                        // student show evaluation
                        if ((block_exacomp_additional_grading(BLOCK_EXACOMP_TYPE_DESCRIPTOR, $data->courseid) && $level == 1)
                            || (block_exacomp_additional_grading(BLOCK_EXACOMP_TYPE_DESCRIPTOR_CHILD, $data->courseid) && $level == 2)) {
                            switch ($data->scheme) {
                                case BLOCK_EXACOMP_ASSESSMENT_TYPE_NONE: // None.
                                    $teacher_evaluation_cell->text = '';
                                    break;
                            ...
                            if (@$data->subject->isglobal && block_exacomp_get_settings_by_course($COURSE->id)->isglobal && block_exacomp_is_dakora_teacher($USER->id)) {
                                if (array_key_exists($descriptor->id, @$student->competencies->globalgradings) && @$student->competencies->globalgradings[$descriptor->id] != "") {
                                    //Add the other globalgradings as tooltipp
                                    $globalgradings = html_writer::tag('b', ' ' . $this->pix_icon("i/groupevent", @$student->competencies->globalgradings[$descriptor->id]) . ' ',
                                        array('id' => 'globalgradings', 'descrid' => $descriptor->id, 'studentid' => $student->id));
                                    $teacher_evaluation_cell->text .= $globalgradings;
                                } else {
                                    //if the course is global, the subject is global and the teacher is a dakorateacher AND the teacher has not graded anything yet, then the other global gradings have to be found
                            ...
                            if (isset($student->competencies->gradinghistory[$descriptor->id]) && substr_count($student->competencies->gradinghistory[$descriptor->id], '<br>') > 1) {
                                $gradinghistory = html_writer::tag('b', ' ' . $this->pix_icon("i/duration", $student->competencies->gradinghistory[$descriptor->id]) . ' ',
                                    array('id' => 'gradinghistory', 'descrid' => $descriptor->id, 'studentid' => $student->id));
                                $teacher_evaluation_cell->text .= $gradinghistory;
                            }
                        }

                        if ((block_exacomp_get_assessment_comp_SelfEval($data->courseid) == 1 && $level == 1)
                            || (block_exacomp_get_assessment_childcomp_SelfEval($data->courseid) == 1 && $level == 2)) {
                            // Only emojis?
                            $student_evaluation_cell->text = $this->generate_select(
                                $checkboxname,
                                $descriptor->id,
                ...
                $checkboxname = "dataexamples";
                $example_scheme = block_exacomp_get_assessment_example_scheme($data->courseid);
                if (!$profoundness && !$forReport) { // TODO: does profoundness need examples?
                    foreach ($descriptor->examples as $example) {
                        ...
                        foreach ($students as $student) {
                            ...
                            if ($this->useEvalNiveau && block_exacomp_get_assessment_example_diffLevel($data->courseid) == 1) {
                                $niveau_cell->text = $this->generate_niveau_select(
                                    'niveau_examples',
                                    $example->id,
                                    'examples',
                            ...
                            //student show evaluation
                            if (block_exacomp_additional_grading(BLOCK_EXACOMP_TYPE_EXAMPLE, $data->courseid)) {
                                switch ($example_scheme) {
                                    case BLOCK_EXACOMP_ASSESSMENT_TYPE_NONE: // None.
                                        $teacher_evaluation_cell->text = '';
                                        break;
                            ...
                            if (block_exacomp_get_assessment_example_SelfEval($data->courseid) == 1) {
                                // Only emojis?
                                $student_evaluation_cell->text = $this->generate_select(
                                    $checkboxname,
                                    $example->id,
                ...
                $child_data->scheme = block_exacomp_get_assessment_childcomp_scheme($data->courseid);
                ...
                if ($editmode && $parent && !$forReport) {
                    ...
                    $cell->attributes['class'] = 'rg2-indent';
                    $cell->text = html_writer::empty_tag('input',
                        array('exa-type' => 'new-descriptor', 'type' => 'text', 'class' => 'addCompetence', 'placeholder' => block_exacomp_trans(['de:Neue Teilkompetenz', 'en:New child competence']), 'parentid' => $descriptor->id));
                    $cell->text .= html_writer::empty_tag('input', array('exa-type' => 'new-descriptor', 'type' => 'button', 'value' => block_exacomp_get_string('add')));
                    $own_additionRow->cells[] = $cell;
                    //					}
                    $own_additionRow->cells[] = new html_table_cell();
        ...
        foreach ($subjects as $subject) {
            ...
            if ($first) {
                ...
                if ($crosssubjid) {
                    ...
                    $title->text = ($usesubjectgrading) ? '' : html_writer::tag("b", s($subject->title));
                }
    ...
    public function source_info($sourceid) {
        ...
        } else if ($sourceid && $source = $DB->get_record(BLOCK_EXACOMP_DB_DATASOURCES, array('id' => $sourceid))) {
            $info = s($source->name);
        }
        if (empty($info)) {
    ...
    public function sources() {
        ...
        foreach ($sources as $source) {
            $name = s($source->name ? $source->name : $source->source);
            $ret .= $this->box(
        ...
        if ($comptype == BLOCK_EXACOMP_TYPE_EXAMPLE) {
            if (!$isTeacher) {
                ...
                if ($studentid && $forSelf) {
                    ...
                    //this creates the link to the submission (square)
                    $itemExists = block_exacomp_get_current_item_for_example($studentid, $compid);
                    $result = html_writer::link(
                        new moodle_url('/blocks/exacomp/example_submission.php', array("courseid" => $courseid, "exampleid" => $compid)),
                        $this->pix_icon((!$itemExists) ? "i/manual_item" : "i/reload", block_exacomp_get_string('submission')),
                        array('exa-type' => 'iframe-popup'));
                }
            } else if ($studentid) {
    ...
    public function visibility_icon_example_solution($visible, $exampleid) {
        if ($visible) {
            $icon = html_writer::empty_tag('img',
                array('src' => new moodle_url('/blocks/exacomp/pix/solution_visible.png'), 'alt' => block_exacomp_get_string("hide_solution"), 'title' => block_exacomp_get_string("hide_solution"), 'width' => '16'));
        } else {
            $icon = html_writer::empty_tag('img',
                array('src' => new moodle_url('/blocks/exacomp/pix/solution_hidden.png'), 'alt' => block_exacomp_get_string("show_solution"), 'title' => block_exacomp_get_string("show_solution"), 'width' => '16'));
        }
    ...
    public function visibility_icon_example_solution_disabled() {
        return html_writer::empty_tag('img',
            array('src' => new moodle_url('/blocks/exacomp/pix/locked.png'), 'alt' => block_exacomp_get_string("hide_solution_disabled"), 'title' => block_exacomp_get_string("hide_solution_disabled"), 'width' => '16'));
    }
    ...
    public function student_evaluation($showevaluation, $isTeacher = true, $niveauid = SHOW_ALL_NIVEAUS, $subjectid = 0, $topicid = -1, $studentid = 0) {
        ...
        $link = new moodle_url("/blocks/exacomp/assign_competencies.php",
            array("courseid" => $COURSE->id, "showevaluation" => (($showevaluation) ? "0" : "1"), 'niveauid' => $niveauid, 'topicid' => $topicid, 'studentid' => $studentid, 'subjectid' => $subjectid));
    ...
    public function generate_select($name, $compid, $type, $student, $evaluation, $scheme, $disabled = false, $profoundness = false, $reviewerid = null) {
        ...
        if (strcmp($evaluation, 'teacher') == 0) {
            $options = global_config::get_teacher_eval_items(0, false, $scheme);
        } else {
            $options = global_config::get_student_eval_items(true, $type);
        }
        if ($this->is_print_mode()) {
    ...
    public function generate_niveau_select($name, $compid, $type, $student, $disabled = false, $reviewerid = null) {
        ...
        if ($this->useEvalNiveau) {
            $options = global_config::get_evalniveaus(true);
    ...
    public function edit_config($data, $courseid, $fromimport = 0) {
        ...
        foreach ($data->levels as $levelstruct) {
            ...
            $cell = new html_table_cell();
            $cell->colspan = 1;
            $cell->text = html_writer::tag('b', s($levelstruct->level->title)) . ' (' . $this->source_info($levelstruct->level->source) . ')';
            ...
            foreach ($levelstruct->schooltypes as $schooltypestruct) {
                $row = new html_table_row();
                $cell = new html_table_cell();
                $cell->text = s($schooltypestruct->schooltype->title);
                $row->cells[] = $cell;
        ...
        if ($taxonomies && count($taxonomies) > 0) {
            ...
            foreach ($taxonomies as $taxkey => $taxonomy) {
                if ($currentsourse != $taxonomy->sourcename) {
                    ...
                    $row = new html_table_row();
                    $row->cells[] = html_writer::tag('strong', s($currentsourse));
                    $rows[] = $row;
                }
                $row = new html_table_row();
                $row->cells[] = s($taxonomy->title);
                $rows[] = $row;
            }
            $table->data = $rows;
    ...
    public function edit_course($settings, $courseid, $headertext) {
        ...
        $input_descriptors =
            html_writer::checkbox('show_all_descriptors', 1, $settings->show_all_descriptors == 1, '&nbsp;' . block_exacomp_get_string('show_all_descriptors'), ($settings->uses_activities != 1) ? array("disabled" => "disabled") : array())
            . html_writer::empty_tag('br');
    ...
    public function my_badges($badges, $onlygained = false) {
        ...
        if ($badges->issued) {
            ...
            foreach ($badges->issued as $badge) {
                ...
                $innerdiv = html_writer::div(s($badge->name));
        ...
        if (!$onlygained) {
            if ($badges->pending) {
                ...
                foreach ($badges->pending as $badge) {
                    ...
                    $imageurl = moodle_url::make_pluginfile_url($context->id, 'badges', 'badgeimage', $badge->id, '/', 'f1', false);
                    $img = html_writer::empty_tag('img', array('src' => $imageurl, 'class' => 'badge-image'));
                    $innerdiv = html_writer::div(s($badge->name), "", array('style' => 'font-weight: bold;'));
                    $innerdiv2 = "";
    ...
    public function courseselection($schooltypes, $active_topics, $headertext, $courseid = 0) {
        ...
        if (count($schooltypes)) {
            ...
            foreach ($schooltypes as $schooltype) {
                ...
                $cell->text =
                    html_writer::div(html_writer::tag('b', s($schooltype->title)) . ' (' . $this->source_info($schooltype->source) .
                        ')');
                ...
                foreach ($schooltype->subjects as $subject) {
                    ...
                    $cell = new html_table_cell();
                    $cell->text = html_writer::div(html_writer::span(s($subject->title), 'rg2-arrow-highlight') .
    ...
    public function descriptor_selection_export() {
        ...
        foreach ($subjects as $subject) {
            ...
            $cell = new html_table_cell();
            $cell->text = html_writer::div('<input type="checkbox" name="subjects[' . $subject->id . ']" value="' . $subject->id . '" />' . html_writer::tag('b', s($subject->title)));
            $cell->attributes['class'] = 'rg2-arrow';
            $row->cells[] = $cell;
            $rows[] = $row;
            ...
            foreach ($subject->topics as $topic) {
                ...
                $cell->text = html_writer::div('<input type="checkbox" name="topics[' . $topic->id . ']" value="' . $topic->id . '" ">' . $topic->numbering . ' ' . s($topic->title), "desctitle");
                $row->cells[] = $cell;
                ...
                foreach ($topic->descriptors as $descriptor) {
                    ...
                    $cell = new html_table_cell();
                    $cell->attributes['class'] = 'rg2-arrow rg2-indent';
                    $cell->text = html_writer::div('<input type="checkbox" name="descriptors[' . $descriptor->id . ']" value="' . $descriptor->id . '" />' . $descriptor->numbering . ' ' . s($descriptor->title), "desctitle");
                    $row->cells[] = $cell;
    ...
    public function descriptor_selection_source_delete($source, $edulevels) {
        ...
        foreach ($edulevels as $edulevel) {
            ...
            $cell = new html_table_cell();
            $cell->text = html_writer::tag('b', s($edulevel->title)) . ' (' . $this->source_info($edulevel->source) . ')';
            $row->cells[] = $cell;
            $rows[] = $row;
            foreach ($edulevel->schooltypes as $schooltype) {
                ...
                foreach ($schooltype->subjects as $subject) {
                    ...
                    $cell->text = html_writer::div('<input type="checkbox"
                        ...
                        ' />' .
                        html_writer::tag('b', s($subject->title)) . $notes);
                    $cell->attributes['class'] = 'rg2-arrow';
                    $row->cells[] = $cell;
                    $rows[] = $row;
                    ...
                    foreach ($subject->topics as $topic) {
                        ...
                        $cell->text = html_writer::div('<input type="checkbox"
                            ...
                            ' />' .
                            $topic->numbering . ' ' . s($topic->title) . $notes,
                            "desctitle");
                        $row->cells[] = $cell;
                        ...
                        foreach ($topic->descriptors as $descriptor) {
                            ...
                            $cell->text = html_writer::div('<input type="checkbox"
                                ...
                                ' />' .
                                $descriptor->numbering . ' ' . s($descriptor->title) . $notes,
                                "desctitle");
                            $row->cells[] = $cell;
                            ...
                            foreach ($descriptor->children as $child_descriptor) {
                                ...
                                $cell->text = html_writer::div('<input type="checkbox"
                                    ...
                                    ' />' .
                                    $child_descriptor->numbering . ' ' . s($child_descriptor->title) . $notes,
                                    "desctitle");
                                $row->cells[] = $cell;
                                ...
                                foreach ($child_descriptor->examples as $example) {
                                    ...
                                    $cell->text = html_writer::div('<input type="checkbox"
                                        ...
                                        ' />' .
                                        $example->numbering . ' ' . s($example->title) . $notes,
                                        "desctitle");
                                    $row->cells[] = $cell;
    ...
    public function subject_preselection_source_delete($source, $edulevels, $courseid) {
            ...
            foreach ($edulevel->schooltypes as $schooltype) {
                ...
                foreach ($schooltype->subjects as $subject) {
                    ...
                    $cell->text = html_writer::div('<input type="checkbox"
                        ...
                        ($checked ? ' checked' : '') .
                        ' /> ' .
                        html_writer::tag('span', s($subject->title))
                        . $preselect_because_disabled
                        . $preselect_because_missing_from_import);
                    $cell->attributes['class'] = 'rg2-arrow';
                    $row->cells[] = $cell;
        ...
        $table_html .= html_writer::div(html_writer::empty_tag('input',
            array('type' => 'submit',
                'value' => block_exacomp_get_string("select_subjects"))),
            '', array('id' => 'exabis_save_button'));
    ...
    public function topics_courseselection(&$rows, $level, $topics, $active_topics) {
        foreach ($topics as $topic) {
            ...
            $outputnameCell = new html_table_cell();
            $outputnameCell->attributes['class'] = 'rg2-arrow rg2-indent';
            $outputnameCell->text = html_writer::div(s($topic->title), "desctitle");
            $topicRow->cells[] = $outputnameCell;
    ...
    public function activity_content($subjects, $modules, $courseid = 0) {
        ...
        foreach ($modules as $module) {
            ...
            $moduleLink = html_writer::link(block_exacomp_get_activityurl($module), s($moduleName), ['title' => $module->name]);
            $quizLink = html_writer::link(new moodle_url($CFG->wwwroot . "/blocks/exacomp/question_overview.php?courseid=" . $COURSE->id . "&moduleid=" . $module->id), s($moduleName), ['title' => $module->name]);
            if (count($modules) > 5) {
            ...
            $downloadTag = html_writer::tag("img", "", array("src" => new moodle_url('/blocks/exacomp/pix/ec_download_bl.png'), "alt" => block_exacomp_get_string('download_activites'), 'height' => '12'));
        ...
        foreach ($subjects as $subject) {
            ...
            $cell->text = html_writer::tag('b', s($subject->title));
            $row->cells[] = $cell;
            $rows[] = $row;
    ...
    public function badge($badge, $descriptors, $context) {
        ...
        $content .= html_writer::div(s($badge->name), '', array('style' => 'font-weight:bold;'));
        ...
        } else if ($badge->status == BADGE_STATUS_ACTIVE) {
        ...
        if ($descriptors) {
            $li_desc = '';
            foreach ($descriptors as $descriptor) {
                $li_desc .= html_writer::tag('li', s($descriptor->title));
            }
    ...
    public function edit_badges($subjects, $badge) {
        ...
        foreach ($subjects as $subject) {
            ...
            $cell = new html_table_cell();
            $cell->attributes['class'] = 'ec_tableheadwidth';
            $cell->text = html_writer::link(new moodle_url('/badges/edit.php', array('id' => $badge->id, 'action' => $badgeaction)), s($badge->name));
            $row->cells[] = $cell;
            $rows[] = $row;
    ...
    function box_error($message) {
        ...
        } else if ($message instanceof moodle_exception) {
    ...
    function competence_profile_course($course, $student, $showall = true, $max_scheme = 3, $forGlobalReport = false, $crosssubj = null, $withoutHeaders = false) {
        ...
        if ($forGlobalReport) { // GLOBAL REPORT
            ...
            foreach ($students as $student) {
                ...
                if ($this->is_print_mode()) {
                    ...
                    $tempTable->attributes['exa-subjectid'] = '-1';
                    $tempTable->attributes['exa-courseid'] = '-1';
                    $row = new html_table_row();
            ...
            foreach ($competence_tree as $subject) {
                $content .= html_writer::tag("h4", s($subject->title), array("class" => "competence_profile_coursetitle"));
        ...
        if (!$withoutHeaders) {
            ...
            $courseNameWrapped = html_writer::tag('h3', s($cn), array('class' => 'competence_profile_coursename'));
        }
    ...
    private function competence_profile_grid($courseid, $subject, $studentid, $max_scheme, $custom_data = null, $crosssubj = null) {
        ...
        foreach ($table_content->content as $topic => $rowcontent) {
            ...
            $cell = new html_table_cell ();
            $cell->text = block_exacomp_get_topic_numbering($topic) . " " . s($table_column[$topic]->title);
            ...
            foreach ($rowcontent->niveaus as $niveau => $element) {
                ...
                if (isset($element->show) && $element->show) {
                    ...
                    if ((
                            (block_exacomp_get_assessment_comp_scheme($courseid) == BLOCK_EXACOMP_ASSESSMENT_TYPE_GRADE &&
                                ($element->eval > 0 || block_exacomp_get_assessment_diffLevel(BLOCK_EXACOMP_TYPE_DESCRIPTOR, $courseid) && $element->evalniveau)
            ...
            if (block_exacomp_is_topicgrading_enabled($courseid)) {
                ...
                if (block_exacomp_additional_grading(BLOCK_EXACOMP_TYPE_TOPIC, $courseid) == BLOCK_EXACOMP_ASSESSMENT_TYPE_GRADE) { // TODO: TOPIC?
                    $rowcontent->topic_eval = global_config::get_additionalinfo_value_mapping($rowcontent->topic_eval);
                }
                ...
                if ((
                        (block_exacomp_get_assessment_topic_scheme($courseid) == BLOCK_EXACOMP_ASSESSMENT_TYPE_GRADE &&
                            ($rowcontent->topic_eval > 0 || block_exacomp_get_assessment_diffLevel(BLOCK_EXACOMP_TYPE_TOPIC, $courseid) && $rowcontent->topic_evalniveau)
        ...
        if (block_exacomp_is_subjectgrading_enabled($courseid)) {
            ...
            $subject_eval_cell->text = $table_content->subject_evalniveau . ($table_content->subject_eval > -1 ? $table_content->subject_eval : '');
    ...
    function subject_statistic_table($courseid, $stat, $stat_title, $showdifflevel = true, $assessmentScheme = null) {
        ...
        $value_titles = global_config::get_teacher_eval_items($courseid, false, $assessmentScheme);
        $value_titles = array_filter($value_titles, 'strlen'); // remove empty, but leave '0'
        $count_values = count($value_titles);
        $value_titles_long = global_config::get_teacher_eval_items($courseid, false);
    ...
    private function comparison_table($courseid, $subject, $student) {
        ...
        //first subject title cell
        $cell = new html_table_cell();
        $cell->text = s($subject->title); //TODO not hardcoded
        ...
        foreach ($subjects as $subject) {
            if (isset($subject->crosssub_drafts)) {
                ...
                foreach ($subject->crosssub_drafts as $draft) {
                    ...
                    $draft_content .= html_writer::start_tag('li');
                    $draft_content .= html_writer::span(html_writer::checkbox('draft[' . $draft->id . ']', $draft->id, false, s($draft->title)), '', array('title' => $text));
    ...
    /**
     * @param cross_subject $cross_subject
     * @param $students
    ...
    public function crosssub_subject_dropdown($crosssubject) {
        ...
        foreach ($subjects as $subject) {
            $options[$subject->id] = s($subject->title);
        }
    ...
    public function overview_metadata_cross_subjects($crosssubject, $edit) {
        ...
        if ($edit) {
            ...
            $cellText = html_writer::tag('div', ($crosssubject) ? s($crosssubject->title) : '');
        }
        ...
        if ($edit) {
            ...
            $cellText = html_writer::tag('b', s($subject_title));
        }
    ...
    public function print_crosssubjects_and_examples($crosssubjects, $isTeacher, $editmode, $show_examples = true) {
        ...
        foreach ($crosssubjects as $crosssubject) {
            ...
            $html_tree .= s($crosssubject->title);
            ...
            foreach ($examples as $example) {
                ...
                $html_tree .= html_writer::tag("li", s($example->title) . $exampleIcons);
            }
            ...
            if ($subject->associated == 1 || ($isTeacher && $editmode == 1)) {
                ...
                foreach ($subject->topics as $tkey => $topic) {
                    ...
                    if ($topic->associated == 1 || ($isTeacher && $editmode == 1)) {
                        ...
                        $html_tree .= block_exacomp_get_topic_numbering($topic->id) . ' ' . s($topic->title);
    ...
    private function competence_for_list_tree($descriptor, $isTeacher, $editmode, $show_examples) {
        ...
        $title = block_exacomp_get_descriptor_numbering($descriptor) . ' ' . s($descriptor->title);
        ...
        if ($show_examples && $descriptor->direct_associated == 1) {
            ...
            foreach ($descriptor->examples as $example) {
                ...
                if ($example->associated == 1 || ($isTeacher && $editmode == 1)) {
                    ...
                    $html_tree .= html_writer::tag("li", s($example->title) . $exampleIcons, array('class' => ($example->associated == 1) ? "associated" : ""));
                }
            }
        }
        if (!empty($descriptor->children)) {
    ...
    public function example_pool($examples = array()) {
        ...
        foreach ($examples as $example) {
            $content .= html_writer::div(s($example->title), 'fc-event', array('exampleid' => $example->exampleid));
        }
    ...
    public function example_trash($trash_examples = array(), $persistent_trash = true) {
        ...
        foreach ($trash_examples as $example) {
            $content .= html_writer::div(s($example->title), 'fc-event');
        }
            ...
            if ($taxonomies && count($taxonomies) > 0) {
                ...
                foreach ($taxonomies as $taxonomy) {
                    if ($currentsourse != $taxonomy->sourcename) {
                        ...
                        $currentsourse = $taxonomy->sourcename;
                        $input_taxonomies .= '<optgroup label="' . s($currentsourse) . '">';
                    }
                    $attributes = array();
    ...
    public function example_based_list_tree($examples, $tableid = '', $courseId = 0, $studentId = 0, $withActivitiesResult = false) {
        ...
        foreach ($examples as $example) {
            ...
            $content .= s($example->title) . ' ' . $exampleIcons;
    ...
    public function pre_planning_storage_students($students, $examples, $groups) {
        ...
        foreach ($groups as $group) {
            ...
            $content .= html_writer::link(new moodle_url('/blocks/exacomp/weekly_schedule.php', array('courseid' => $COURSE->id, 'studentid' => 0)),
                s($group->name), array('target' => '_blank', 'title' => block_exacomp_get_string('to_weekly_schedule')));
        ...
        $content .= html_writer::tag('span',
            html_writer::start_tag('fieldset', array('class' => 'gray')) . html_writer::end_tag('fieldset') . block_exacomp_trans(['de:Material aus Vorplanungsspeicher erhalten', 'en:Examples from pre-planning storage']),
            array('class' => 'pre_planning_storage_legend_gray'));
        $content .= html_writer::tag('span', html_writer::start_tag('fieldset', array('class' => 'blue')) . html_writer::end_tag('fieldset') . block_exacomp_trans(['de:Noch kein Material erhalten', 'en:No examples received']),
            array('class' => 'pre_planning_storage_legend_blue'));
    ...
    public function cross_subjects_overview_student($course_crosssubs) {
        ...
        foreach ($course_crosssubs as $crosssub) {
            $content .= html_writer::link(new moodle_url('/blocks/exacomp/cross_subjects.php', array('courseid' => g::$COURSE->id, 'crosssubjid' => $crosssub->id)), s($crosssub->title));
            $content .= html_writer::empty_tag('br');
        }
    ...
    public function cross_subjects_overview_teacher($course_crosssubs) {
        ...
        foreach ($course_crosssubs as $crosssub) {
            ...
            $title->text = html_writer::link(new moodle_url('/blocks/exacomp/cross_subjects.php', array('courseid' => g::$COURSE->id, 'crosssubjid' => $crosssub->id)), s($crosssub->title));
            $table->data[] = [
                $title,
        ...
        foreach ($course_crosssubs as $crosssub) {
            ...
            $table->data[] = [
                $title,
                html_writer::link(new moodle_url('/blocks/exacomp/cross_subjects.php', array('courseid' => g::$COURSE->id, 'crosssubjid' => $crosssub->id, 'editmode' => 1)), $this->pix_icon("i/edit", block_exacomp_get_string("edit"))) .
                html_writer::link('#', $this->pix_icon("t/delete", block_exacomp_get_string("delete")),
        ...
        foreach ($subjects as $subject) {
            $title = new html_table_cell;
            $title->text = '<div>' . s($subject->title) . '</div>';
            ...
            foreach ($groupcategories as $groupcategory) {
                ...
                while ($crosssubj && $groupcategory == $crosssubj->groupcategory && $crosssubj->id != null) {
                    ...
                    $title = clone $item_title_cell;
                    $title->text = html_writer::link(new moodle_url('/blocks/exacomp/cross_subjects.php', array('courseid' => g::$COURSE->id, 'crosssubjid' => $crosssubj->id)), s($crosssubj->title));
                    $row = new html_table_row([
                        $title,
                        ($crosssubj->has_capability(BLOCK_EXACOMP_CAP_MODIFY) ? html_writer::link(new moodle_url('/blocks/exacomp/cross_subjects.php', array('courseid' => g::$COURSE->id, 'crosssubjid' => $crosssubj->id, 'editmode' => 1)),
                            $this->pix_icon("i/edit", block_exacomp_get_string("edit"))) : '') .
    ...
    function studentselector($students, $selected, $option = null, $groups = null, $attributes = []) {
        ...
        if (is_array($groups) && count($groups) > 0) {
            foreach ($groups as $group) {
                $studentsAssociativeArray[-($group->id + 1)] = s($group->name);
            }
        }
        ...
        <form method="post" action="<?php echo $action; ?>">
            ...
            <div class="filter-group <?php echo $isTeacher ? 'visible' : '' ?>">
                ...
                <div class="filter-group-body">
                    <div>
                        <label><input type="radio" name="filter[type]" value="students" <?php if (@$filter['type'] == 'students') {
                        ...
                        <br><label><input type="radio" name="filter[type]" value="student_counts" <?php if (@$filter['type'] == 'student_counts') {
                                echo 'checked="checked"';
                            } ?>/>
    ...
    function group_report_annex_filters($type, $filter, $action, $extra, $courseid, $isTeacher) {
        ...
        <form method="post" action="<?php echo $action; ?>">
            ...
            if (block_exacomp_get_assessment_subject_scheme($courseid) == block_exacomp_get_assessment_comp_scheme($courseid)) {
                $this->group_reports_print_filter($filter, BLOCK_EXACOMP_TYPE_SUBJECT, 'report_subject');
            }
            // Topic can only be chosen if the grading scheme configured is the same as competence.
            if (block_exacomp_get_assessment_topic_scheme($courseid) == block_exacomp_get_assessment_comp_scheme($courseid)) {
                $this->group_reports_print_filter($filter, BLOCK_EXACOMP_TYPE_TOPIC, 'report_competencefield');
            }
            $this->group_reports_print_filter($filter, BLOCK_EXACOMP_TYPE_DESCRIPTOR_PARENT, 'descriptor');
            //$this->group_reports_print_filter($filter, BLOCK_EXACOMP_TYPE_DESCRIPTOR, 'descriptor');
            // Child-competence can only be chosen if the grading scheme configured is the same as competence.
            if (block_exacomp_get_assessment_comp_scheme($courseid) == block_exacomp_get_assessment_childcomp_scheme($courseid)) {
                $this->group_reports_print_filter($filter, BLOCK_EXACOMP_TYPE_DESCRIPTOR_CHILD, 'descriptor_child');
            }
            //$this->group_reports_print_filter($filter, BLOCK_EXACOMP_TYPE_EXAMPLE, 'report_learniningmaterial');
    ...
    private function group_reports_print_filter($filter, $input_type, $titleid) {
        ...
        $inputs = global_config::get_allowed_inputs($input_type);
    ...
    function radar_graph($records) {
        ...
        <script>
            var radarChartData = {
                labels: <?php echo json_encode(array_values(array_map(function($a) {
                    return $a->title;
                }, $records))); ?>,
                datasets: [{
                    ...
                    data: <?php echo json_encode(array_values(array_map(function($a) {
