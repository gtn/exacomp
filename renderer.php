                    ...
                    if(!$statistic){
                        foreach($students as $student) {
                            ...
                            */
							...
							$additional_grading_cell->text = html_writer::select(range(0, 100),'additionalinfo',$student_additional_grading,false,array('id'=>'additionalinfo-'.$student->id.'-'.$example->id.'-'.$descriptor->id,'exampleid'=>$example->id,'studentid'=>$student->id, 'disabled'=>($visible_student_example) ? '' : 'disabled')); 
							// html_writer::div(html_writer::div('','slider-'.$student->id.'-'.$example->id.'-'.$descriptor->id),'dialog-'.$student->id.'-'.$example->id.'-'.$descriptor->id);
