...
$form = new block_exacomp_example_upload_student_form($_SERVER['REQUEST_URI'],
    array("taxonomies" => $taxonomies, "tree" => $tree, "exampleid" => $exampleid, "task" => ($task = block_exacomp_get_file_url($example, 'example_task')) ? $task : null,
...
if ($formdata = $form->get_data()) {
    ...
    $itemid = $DB->insert_record("block_exaportitem",
        array('userid' => $USER->id, 'name' => $formdata->title, 'url' => $formdata->externalurl, 'intro' => $formdata->description, 'type' => $type, 'timemodified' => time(), 'categoryid' => $subject_category->id));
