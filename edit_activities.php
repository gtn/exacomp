...
if (($action = optional_param("action", "", PARAM_TEXT) )== "save") {
	...
	if(!isset($_POST['data']) && !isset($_POST['topicdata']))
		...
		$headertext=block_exacomp_get_string("save_success", "block_exacomp") .html_writer::empty_tag('br')
			...
			.block_exacomp_get_string('completed_config', 'block_exacomp');
	...
	$headertext = html_writer::empty_tag('img', array('src'=>$img, 'alt'=>'', 'width'=>'60px', 'height'=>'60px'))
		.block_exacomp_get_string('teacher_third_configuration_step', 'block_exacomp')
		.html_writer::link(new moodle_url('/blocks/exacomp/edit_course.php', array('courseid'=>$courseid)), block_exacomp_get_string('teacher_third_configuration_step_link', 'block_exacomp'));
}
