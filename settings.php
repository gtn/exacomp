	...
	class block_exacomp_admin_setting_scheme extends admin_setting_configselect {
		public function write_setting($data) {
			...
			foreach($titles as $index => $title){
				$record = $DB->get_record(BLOCK_EXACOMP_DB_EVALUATION_NIVEAU, array('id'=>$index));
				if(!$record){
					$entry = new stdClass();
					$entry->title = $title;
					$DB->insert_record(BLOCK_EXACOMP_DB_EVALUATION_NIVEAU, $entry);
				}else{
					$record->title = $title;
					$DB->update_record(BLOCK_EXACOMP_DB_EVALUATION_NIVEAU, $record);
				}
			}
...
$settings->add(new block_exacomp_admin_setting_scheme('exacomp/adminscheme', block_exacomp_get_string('settings_admin_scheme'),
	block_exacomp_get_string('settings_admin_scheme_description'), block_exacomp_get_string('settings_admin_scheme_none'), array(block_exacomp_get_string('settings_admin_scheme_none'), 'G/M/E', 'A/B/C', '*/**/***')));
