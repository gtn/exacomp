	...
	if(empty($topics)) {
		...
		if($course_settings->uses_activities){
			...
			$headertext=block_exacomp_get_string("save_success", "block_exacomp") .html_writer::empty_tag('br')
				...
				. html_writer::link(new moodle_url('edit_activities.php', array('courseid'=>$courseid)), block_exacomp_get_string('next_step', 'block_exacomp'));
		}else{
			 $headertext=block_exacomp_get_string("save_success", "block_exacomp") .html_writer::empty_tag('br')
				.html_writer::empty_tag('img', array('src'=>$img, 'alt'=>'', 'width'=>'60px', 'height'=>'60px')).block_exacomp_get_string('completed_config', 'block_exacomp');
	
	   		 $students = block_exacomp_get_students_by_course($courseid);
	   		 if(empty($students))
				$headertext .= html_writer::empty_tag('br')
					.html_writer::link(new moodle_url('/enrol/users.php', array('id'=>$courseid)), block_exacomp_get_string('optional_step', 'block_exacomp'));
		}
	}
}else{
	$headertext = html_writer::empty_tag('img', array('src'=>$img, 'alt'=>'', 'width'=>'60px', 'height'=>'60px')).block_exacomp_get_string('teacher_second_configuration_step', 'block_exacomp');
}
