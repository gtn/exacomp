...
function block_exacomp_get_activities($descid, $courseid = null) { //alle assignments die einem bestimmten descriptor zugeordnet sind
    global $CFG, $DB;
    $query = "SELECT a.id,ass.grade FROM {block_exacompdescriptors} descr INNER JOIN {block_exacompdescractiv_mm} mm  ON descr.id=mm.descrid INNER JOIN {course_modules} a ON a.id=mm.activityid LEFT JOIN {assignment} ass ON ass.id=a.instance  ";
...
function block_exacomp_get_subjects_by_id($subids) {
    ...
    $query = 'SELECT s.title, s.id FROM {block_exacompsubjects} s WHERE s.id IN (' . $comma_separated . ')';
    $subjects = $DB->get_records_sql($query);
...
function block_exacomp_get_activity_icon($descriptorid) {
    ...
    foreach ($activities as $activity) {
        ...
        $desc = get_coursemodule_from_id('assignment', $activity->activityid);
...
function block_exacomp_get_student_icon($activities, $student) {
    ...
    foreach ($activities as $activity) {
        ...
        $act = get_coursemodule_from_id('assignment', $activity->id);
        if(!$act) continue;
        
        //$submission = $DB->get_record('assignment_submissions', array("userid" => $student->id, "assignment" => $act->instance));
        $query="SELECT s.*, a.grade as agrade FROM {assignment_submissions} s INNER JOIN {assignment} a ON s.assignment=a.id WHERE a.id=".$act->instance." AND s.userid=".$student->id;
