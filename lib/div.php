...
function block_exacomp_xml_exists() {
	...
	return file_exists($CFG->dirroot."/blocks/exacomp/xml/exacomp_data.xml");
}
...
function block_exacomp_get_student_icon($activities, $student,$courseid,$gradelib=false) {
	...
	foreach ($activities as $activity) {
		...
		if ($mod->name == "assign" || $mod->name == "quiz") {
			if($mod->name == "assign" && $DB->get_field("assign","nosubmissions",array("id"=>$act->instance)) == 0)
				$actSubOccured = true;
		}
		...
		foreach($rs as $submission){//hoechstens 1 durchlauf moeglich
			if ($submission) {
				...
				if ($mod->name == "assignment" && $submission->grade>=0){
					$submitted .=  get_string('bewertung', "block_exacomp")."(" . $submission->grade . "/".$submission->agrade.")";
				} else if($mod->name == "assign") {
					$grade = $DB->get_record("assign_grades", array("assignment"=>$act->instance,"userid"=>$student->id));
					if($grade)
						$submitted .=  get_string('bewertung', "block_exacomp")."(" . round($grade->grade,2) . "/".$submission->agrade.")";
				}
				$submitted .= "</li>";
				$count = true;
			}
